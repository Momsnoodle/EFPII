module Main #(
  parameter DIVIDER_LEN = 500000, //Number of Clk cycles to wait
...
always_ff @(posedge clk) begin // this will let it blink --> the tick is only on for one clk every --> for me to check if someting is running
    ...
    if (tick_slow) begin
        tick_slow_stay <= ~tick_slow_stay; // will hold the state after one tick until the next tick will turn it off!
    end
    if (tick_fast) begin
        tick_fast_stay <= ~tick_fast_stay; // wilsl hold the state after one tick until the next tick will turn it off!
    end

     if (tick_countdown_stay) begin //countdown tick
        LEDR[8] <= 1; // will hold the state after one tick until the next tick will turn it off!
    end
    else begin
        LEDR[8] <= 0; // will hold the state after one tick until the next tick will turn it off!
    end

       if (tick_slow_stay) begin //countdown tick, give the rhytm
        LEDR[1] <= 1; // will hold the state after one tick until the next tick will turn it off!
    end
    else begin
        LEDR[1] <= 0; // will hold the state after one tick until the next tick will turn it off!
    end
    ...
    if(display_done)begin //done displaying the sequence
    LEDR[3]<= 1;
    end
    else begin
      LEDR[3]<= 0;
    ...
    if(state == 4)begin //Playing, not here for long enough...
    LEDR[5]<= 1;
    end
    else begin
      LEDR[5]<= 0;
...
BinBCD #(
  ...
  assign HEX0[7] = 1'b1; // turn off the dot
